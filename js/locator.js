...
var CriterionLocator = (function () {
  ...
  function selectPred(pred) {
    return function (code) {
      return SqlUtil.selectStmts(code)
        .filter(function (s) { return pred(SqlUtil.normalize(s), s); })
        .map(function (s) {
          var idx = code.indexOf(s);
          if (idx < 0) return null;
          return {
            start: SqlUtil.posToLine(code, idx),
            end: SqlUtil.posToLine(code, idx + s.length)
          };
        })
        .filter(Boolean);
    };
  ...
  var locators = {
    ...
    schema_create: function (code) {
      return stmts(code, function (t) {
        var n = SqlUtil.normalize(t);
        return /\bCREATE\s+TABLE\b/i.test(n) && !/\bCREATE\s+TABLE\s+\w+\.\w+/i.test(n);
      });
    },
    ...
    ctas_agents: function (code) { return firstOrAll(code, /\bCREATE\s+TABLE\s+[\w.]*AGENTS\s+AS\b/i); },
    defaults: function (code) { return firstOrAll(code, /\bALTER\s+TABLE\b[\s\S]*\bDEFAULT\b/i); },
    insert_agent: function (code) { return firstOrAll(code, /\bINSERT\s+INTO\s+[\w.]*AGENTS\b/i); },
    supervisor_alter: function (code) { return firstOrAll(code, /\bALTER\s+TABLE\s+[\w.]*AGENTS\s+ADD\b[\s\S]*\bidsupervisor\b/i); },
    ...
    ctas_partners: function (code) { return firstOrAll(code, /\bCREATE\s+TABLE\s+[\w.]*partners\s+AS\b/i); },
    alter_agent: function (code) { return firstOrAll(code, /\bALTER\s+TABLE\s+[\w.]*partners\s+MODIFY\b/i); },
    insert_partner: function (code) { return firstOrAll(code, /\bINSERT\s+INTO\s+[\w.]*partners\b/i); },
    delete_all: function (code) { return firstOrAll(code, /\bDELETE\s+FROM\s+[\w.]*partners\b/i); },
    ...
    create_insuranceTypes: function (code) { return firstOrAll(code, /\bCREATE\s+TABLE\s+[\w.]*InsuranceTypes\b/i); },
    ...
    update_agency: function (code) { return firstOrAll(code, /\bUPDATE\s+[\w.]*agents\b/i); },
    ...
    delete_agents_match: function (code) { return firstOrAll(code, /\bDELETE\s+FROM\s+[\w.]*agents\b/i); },

    ctas_partners_full: function (code) { return firstOrAll(code, /\bCREATE\s+TABLE\s+[\w.]*partners\s+AS\b/i); },
    add_emergency_col: function (code) { return firstOrAll(code, /\bALTER\s+TABLE\s+[\w.]*partners\s+ADD\b/i); },
    update_phone_mail: function (code) { return firstOrAll(code, /\bUPDATE\s+[\w.]*partners\b/i); },
    ...
    update_partner_gaps: function (code) { return firstOrAll(code, /\b(?:UPDATE|MERGE)\s+[\w.]*partners\b/i); },

    q1a_active_policies: selectPred(function (n) {
      return /\bpolic/i.test(n) && /\bproduct/i.test(n) && /\bagent/i.test(n) &&
        /\bactive\b/i.test(n) && SqlUtil.hasInnerJoinSyntax(n);
    }),
    q1b_left_agents: selectPred(function (n, raw) {
      return /\bagent/i.test(n) && /\bpolic/i.test(n) && SqlUtil.hasLeftJoinSyntax(n, raw);
    }),
    q1c_union_counterparties: selectPred(function (n) {
      return SqlUtil.hasUnionSyntax(n) && (/\bagent/i.test(n) || /\bpartner/i.test(n));
    }),
    q1d_intersect: selectPred(function (n) { return SqlUtil.hasIntersectSyntax(n); }),
    ...
    q2a_group_having: selectPred(function (n) {
      return /\bGROUP\s+BY\b/i.test(n) && /\bHAVING\b/i.test(n) && /\bactive\b/i.test(n);
    }),
    ...
    q2b_avg_rating: selectPred(function (n) {
      return /\bAVG\s*\(\s*[\w.]*nrating\s*\)/i.test(n) && /\bpolic/i.test(n);
    }),
    q2c_union_min2: selectPred(function (n) {
      return SqlUtil.hasUnionSyntax(n) && /\bGROUP\s+BY\b/i.test(n) && /\bHAVING\b/i.test(n);
    }),

    q3a_full_join_regions: selectPred(function (n) {
      return SqlUtil.hasFullJoinSyntax(n) && /\bagent/i.test(n) && /\bpartner/i.test(n);
    }),
    q3a_region_columns: selectPred(function (n) {
      return SqlUtil.hasFullJoinSyntax(n) && /\bregion/i.test(n);
    }),
    q3b_cross_join_pairs: selectPred(function (n) {
      return SqlUtil.hasCrossJoinSyntax(n) && /\bpartner/i.test(n) && /\bagent/i.test(n);
    }),
    q3b_name_columns: selectPred(function (n) {
      return SqlUtil.hasCrossJoinSyntax(n) && /\bname\b/i.test(n);
    }),
